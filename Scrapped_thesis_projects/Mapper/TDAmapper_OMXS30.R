...
# http://homepage.divms.uiowa.edu/~idarcy/COURSES/TDA/SPRING17/3900.html
# https://people.clas.ufl.edu/peterbubenik/software/
######
...
library(TDAmapper)
source("C:/Users/extra23/Desktop/Filer/MEX/Local laptop backup/Other Financial Data/TDA/TDAmapper-master_mod/TDAmapper-master/R/mapper1D.R")
source("C:/Users/extra23/Desktop/Filer/MEX/Local laptop backup/Other Financial Data/TDA/TDAmapper-master_mod/TDAmapper-master/R/cluster_cutoff_at_first_empty_bin.R")
require(fastcluster)
set.seed(911117)

#DROP X.OMX
omx$X.OMX <- NULL
windowed <- window(omx, start = as.Date("2014-05-04"),end = as.Date("2017-05-04"))
...
#TDAmapper
m1 <- mapper1D_mod( # 12 nodes, 14
  distance_matrix = D, 
  filter_values = filter,
  num_intervals = 20, #16
  percent_overlap = 40, #40
  num_bins_when_clustering = 6, #6,
  linkage = "single"
)
...
# Remove NA from points in vertices
for (i in 1:m1$num_vertices){
  m1$points_in_vertex[[i]] <- m1$points_in_vertex[[i]][!is.na(m1$points_in_vertex[[i]])]
}
...
# Create list of duplicate vertices
duplicate_vertices <- c()
duplicate_vertices_mirror <- c()
for(i in 1:m1$num_vertices){
  #check diff of two vertices
  for(j in 1:m1$num_vertices){
    check_na <- (!is.na(m1$points_in_vertex[[i]][1]) & !is.na(m1$points_in_vertex[[j]][1]))
    #length(setdiff()) checks how many elements differ between vertices, if == 0, then all are same
    if(length(setdiff(m1$points_in_vertex[[i]],m1$points_in_vertex[[j]]))==0 & 
       length(setdiff(m1$points_in_vertex[[j]],m1$points_in_vertex[[i]]))==0 &
       i != j & 
       check_na){
      if (!(i %in% duplicate_vertices)){
        str <- paste(i,"-", j, sep = "")
        print(str)
        print("")
        duplicate_vertices <- c(duplicate_vertices, i,j)
        duplicate_vertices_mirror <- c(duplicate_vertices_mirror, i)
      }
    }
  }
}
...
#Show only vertices in list PERFECT
#m1$points_in_vertex[c(15,16)]
# Delete all redundancy
g1 <- delete_vertices(g1, c(empty_vertices, duplicate_vertices_mirror))
...
non_red_vertices <- all_vertices[!(all_vertices %in% c(empty_vertices, duplicate_vertices_mirror))]
...
#     # This is wrong - we must make Na 0, or else more NA ==> higher mean !!!!!!!!!!!!!!!!!!!!!!!!!!!
#   }
...
for (i in non_red_vertices){
  points.in.vertex <- m1$points_in_vertex[[i]]
  
  y.mean.vertex <- c(y.mean.vertex, mean(colMeans(data[names(points.in.vertex)])))
}
...
for (i in non_red_vertices){
  points.in.vertex <- m1$points_in_vertex[[i]]
  vertex.size <- c(vertex.size, length(m1$points_in_vertex[[i]]))
}
...
#Show vertices
m1$points_in_vertex[non_red_vertices]
